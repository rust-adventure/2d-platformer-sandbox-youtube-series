...
const BUTTON_OVERLAY_Z_INDEX: f32 = 0.0;
...
impl FromWorld for ButtonMaterials {
    fn from_world(world: &mut World) -> Self {
        let mut materials =
            world.resource_mut::<Assets<ColorMaterial>>();
        Self {
            normal: materials.add(ColorMaterial::from(
                NORMAL_BUTTON_COLOR,
            )),
            active: materials.add(ColorMaterial::from(
                ACTIVE_BUTTON_COLOR,
            )),
        }
    }
...
impl FromWorld for ButtonMeshes {
    fn from_world(world: &mut World) -> Self {
        let mut meshes =
            world.resource_mut::<Assets<Mesh>>();
        Self {
            circle: meshes
                .add(
                    shape::Circle::new(BUTTON_RADIUS)
                        .into(),
                )
                .into(),
            triangle: meshes
                .add(
                    shape::RegularPolygon::new(
                        BUTTON_RADIUS,
                        3,
                    )
                    .into(),
                ...
                .into(),
            start_pause: meshes
                .add(shape::Quad::new(START_SIZE).into())
                .into(),
            trigger: meshes
                .add(shape::Quad::new(TRIGGER_SIZE).into())
                .into(),
        }
    }
}
...
pub struct GamepadVisualizerPlugin;
impl Plugin for GamepadVisualizerPlugin {
...
fn setup_sticks(
    ...
    let mut spawn_stick =
        |x_pos, y_pos, x_axis, y_axis, button| {
            commands
            .spawn_bundle(SpatialBundle {
                transform: Transform::from_xyz(x_pos, y_pos, BUTTON_OVERLAY_Z_INDEX),
                ..default()
            })
            .with_children(|parent| {
                // full extent
    ...
    for gamepad in gamepads.iter() {
        for (mut handle, react_to) in query.iter_mut() {
            if button_inputs.just_pressed(
                GamepadButton::new(*gamepad, **react_to),
            ) {
                *handle = materials.active.clone();
            }
            if button_inputs.just_released(
                GamepadButton::new(*gamepad, **react_to),
            ) {
...
fn update_button_values(
    ...
    for event in events.iter() {
        if let GamepadEventType::ButtonChanged(
            button_type,
            value,
        ) = event.event_type
        {
...
fn update_axes(
    ...
    for event in events.iter() {
        if let GamepadEventType::AxisChanged(
            axis_type,
            value,
        ) = event.event_type
        {
            for (mut transform, move_with) in
                query.iter_mut()
            {
            ...
            for (mut text, text_with_axes) in
                text_query.iter_mut()
            {
