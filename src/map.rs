...
use bevy::{
    asset::{
        AssetLoader, AssetPath, BoxedFuture, LoadContext,
        LoadedAsset,
    },
    prelude::*,
};
...
impl AssetLoader for LdtkLoader {
    fn load<'a>(
        ...
        Box::pin(async move {
            let project: ldtk_rust::Project =
                serde_json::from_slice(bytes)?;
            let dependencies: Vec<(i64, AssetPath)> =
                project
                    .defs
                    .tilesets
                    .iter()
                ...
                tilesets: dependencies
                    ...
                    .map(|dep| {
                        (
                            dep.0,
                            load_context
                                .get_handle(dep.1.clone()),
                        )
                    })
                    .collect(),
            });
            load_context.set_default_asset(
                loaded_asset.with_dependencies(
                    dependencies
                        .iter()
                        .map(|x| x.1.clone())
                        .collect(),
...
pub fn process_loaded_tile_maps(
    ...
    for changed_map in changed_maps.iter() {
        for (_, map_handle, map_config, mut map) in
            query.iter_mut()
        {
            ...
            if let Some(ldtk_map) = maps.get(map_handle) {
                ...
                let default_grid_size =
                    ldtk_map.project.default_grid_size;
                let level = &ldtk_map.project.levels
                    [map_config.selected_level];

                let map_tile_count_x = (level.px_wid
                    / default_grid_size)
                    as u32;
                let map_tile_count_y = (level.px_hei
                    / default_grid_size)
                    as u32;

                let map_size = MapSize(
                    (map_tile_count_x as f32 / 32.0).ceil()
                        as u32,
                    (map_tile_count_y as f32 / 32.0).ceil()
                        as u32,
                );
                    ...
                    .enumerate()
                {
                    ...
                    let settings = LayerSettings::new(
                        ...
                        TextureSize(
                            tileset.px_wid as f32,
                            tileset.px_hei as f32,
                        ),
                    );
                    ...
                    let tileset_width_in_tiles = (tileset
                        .px_wid
                        / default_grid_size)
                        as u32;

                    for tile in layer.grid_tiles.iter() {
                        let tileset_x = (tile.src[0]
                            / default_grid_size)
                            as u32;
                        let tileset_y = (tile.src[1]
                            / default_grid_size)
                            as u32;

                        let mut pos = TilePos(
                            (tile.px[0] / default_grid_size)
                                as u32,
                            (tile.px[1] / default_grid_size)
                                as u32,
                        );
                        // dbg!(
                        //     &tile.px,
                        ...
                        layer_builder
                            .set_tile(
                                pos,
                                Tile {
                                    texture_index: tile.t
                                        as u16, // (tileset_y * tileset_width_in_tiles + tileset_x)
                                    // as u16,
                                    ..Default::default()
                                }
                                .into(),
